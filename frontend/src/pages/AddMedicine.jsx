...
const AddMedicine = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    medicineImage: 'https://via.placeholder.com/300?text=Medicine'
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
        ...
        commonUses: formData.commonUses ? formData.commonUses.split(',').map(s => s.trim()) : [],
        commonSideEffects: formData.commonSideEffects ? formData.commonSideEffects.split(',').map(s => s.trim()) : [],
        alternativeMedicines: formData.alternativeMedicines ? formData.alternativeMedicines.split(',').map(s => s.trim()) : []
      };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-3xl font-bold text-slate-100">Add New Medicine</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-xl p-8 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">Strength (e.g. 500mg)</label>
